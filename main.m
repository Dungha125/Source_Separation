...
% Reset RNG to modern default and set seed reproducible
rng('default');
rng(1,'twister');
...
if evalu %%%read source signals signals
    ...
    audiowrite('stereomix.wav', X', fs);%save stereo mixture, not scaled!!!
else 
...
% use number of time samples when computing power
powpow=10*log10((sum(X(1,:).^2)+sum(X(2,:).^2))/(2*size(X,2)));%power of mixture(used to determine thresholds)
...
while cnt<countmax
    ...
    if(isempty(xbuffer))%%% all branches in the algorithm has ended, no more signal pairs to which ICA should be applied
        ...
        if isequal(lastremmask,remainingmask) %%% Should not continue if background mask have not changed.
            if dis
